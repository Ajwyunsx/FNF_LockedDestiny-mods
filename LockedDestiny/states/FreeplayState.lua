...
function reloadFreeplayList()
    songs = {}
    totalSongs = getFreeplaySongCount() or 0

    debugPrint('Freeplay total songs: '..tostring(totalSongs))

    if totalSongs <= 0 then return end

    for i = 0, totalSongs - 1 do
        ...
        local baseY = listStartY + lineH * i
    ...
    --Images
    makeLuaSprite('Min1', 'Menu/Menu3/SongsMini/Mini1', -580, -350)
    scale('Min1', 0.9, 0.9)
    add('Min1', false)
    set('Min1.color', getColorFromHex('000000'))

    makeLuaSprite('Min4', 'Menu/Menu3/SongsMini/Mini6', -580, 298)
    scale('Min4', 0.9, 0.9)
    add('Min4', false)
    set('Min4.color', getColorFromHex('000000'))
...
doTweenColor('restoreColor', 'Min1', 'FFFFFF', 1, 'quadOut')
doTweenColor('restoreColor2', 'Min4', 'FFFFFF', 1, 'quadOut')
...
function onUpdateOptions(elapsed)
    ...
    if keyJustPressed('accept') then
        ...
        startFreeplaySongIndex(haxeIndex, diffIndex)
    end
